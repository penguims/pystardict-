...
1. implemented stardict dictionary tar file input
2. DictEntity class to wrap dictionary word entity
3. line_encode to deal with problem to read file from tar archive file
   of ifo file. convert from bytes to str object
4. change Dictionary class to StarDict
by magic fang
magicfang@gmail.com
"""
...
class _StarDictIfo(object):
    ...
    def __init__(self, dict_prefix="", container=None, fh=None):
        _file = fh
        # not from tar archive file 
        if not fh:
        ...
        # skipping ifo header
        # when read from tar archive file directly, the ifo file will 
        # read a bytes line
        _line = decode_line(_file.readline()).split('=')
        if _line[0] == 'version':
        ...
        _config = {}
        for _line in _file:
            _line_splited = decode_line(_line).split('=')
...
class _StarDictIdx(object):
    ...
    def __init__(self, dict_prefix="", container=None, fh=None):
        ...
        if not fh:
            ...
            try:
                _file = open_file(idx_filename, idx_filename_gz)
            except Exception as e:
                raise Exception('idx file opening error: "{}"'.format(e))
        self._file = _file.read()
        """ check file size """
        if _file.tell() != container.ifo.idxfilesize:
...
class _StarDictDict(object):
    ...
    def __init__(self, dict_prefix='', container=None, fh=None, in_memory=False):
        """
        ...
        if not fh:
            ...
            try:
                _file = open_file(dict_filename, dict_filename_dz)
            except Exception as e:
...
class DictEntity(object):
    """
        dictionary word entity class:
            word: english word
            word_cn: word in chinese
            phonetic: english phonetics
            commendation: description in english
            value: original word value
    """ 
    # chinese word regrex pattern
    cn_pattern = "[\u4e00-\u9fa5，。；：‘“？！（）【】「」、]+"
    # phonetic regrex pattern, to be refined
    pn_pattern = "/.+/"
    cn_re = re.compile(cn_pattern)
    pn_re = re.compile(pn_pattern)
    # get key and value from StarDict parser
    def __init__(self, key='', value=''):
        ...
        for res in self.pn_re.findall(value):
            {self.phonetic.append(pn) for pn in re.sub('[/\s]+', '', res).split(';')}
        # build chinese word string
        for cn in self.cn_re.findall(value):
            self.word_cn.append(cn)
        # build commendation string
        # delete chinese word in commendation
        en = re.sub(self.cn_pattern, '', value)
        # delete phonetic pattern in commendation
        en = re.sub(self.pn_pattern, '', en)
        # delete empty line
        en = re.sub("[\s\t\n\r]{2,}", "\n", en) 
        en = re.sub("^[\n\r]+", "", en)
        self.commendation = en
    # return entity string in json format
    def __str__(self):
        return '{{\n\t"word": "{}"\n\t"phonetic": "/{}/"\n\t"word_cn": "{}"\n\t"commendation": "{}"\n}}\n'.format(
            self.word,
            "/; /".join(self.phonetic),
            "; ".join(self.word_cn),
            textwrap.wrap(self.commendation, fix_sentence_endings=True),
        )
...
class StarDict(dict):
    ...
    def __init__(self, dict_prefix='', in_memory=False):
        """
        ...
        (ifo_fh, idx_fh, dict_fh, syn_fh)=(None, None, None, None)
        if dict_prefix.endswith('tar.bz2'):
            fh = tarfile.open(dict_prefix, 'r:bz2')
            for tinfo in fh.getmembers():
                mem_fh = fh.extractfile(tinfo)
                if tinfo.name.endswith('.idx'):
                    idx_fh = mem_fh
                elif tinfo.name.endswith('.ifo'):
                    ifo_fh = mem_fh
                elif tinfo.name.endswith('.syn'):
                    syn_fh = mem_fh
                elif tinfo.name.endswith('.dict'):
                    dict_fh = mem_fh
                elif tinfo.name.endswith('.idx.gz'):
                    idx_fh = gzip.GzipFile(fileobj=mem_fh, mode='r')
                elif tinfo.name.endswith('.dict.dz'):
                    dict_fh = gzip.GzipFile(fileobj=mem_fh, mode='r')
        # reading somedict.ifo
        self.ifo = _StarDictIfo(dict_prefix=dict_prefix, container=self, fh=ifo_fh)

        # reading somedict.idx or somedict.idx.gz
        self.idx = _StarDictIdx(dict_prefix=dict_prefix, container=self, fh=idx_fh)

        # reading somedict.dict or somedict.dict.dz
        self.dict = _StarDictDict(
            dict_prefix=dict_prefix, container=self, in_memory=in_memory, fh=dict_fh)

        # reading somedict.syn (optional)
        self.syn = _StarDictSyn(dict_prefix=dict_prefix, container=self, fh=syn_fh)
